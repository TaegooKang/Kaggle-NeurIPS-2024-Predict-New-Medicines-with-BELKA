"""
ComENet implementation.
Source from 'https://github.com/divelab/DIG/blob/dig-stable/dig/threedgraph/method/comenet/comenet.py'
"""
...
class ComENet_Enc(nn.Module):
    ...
    def __init__(
        ...
        super(ComENet_Enc, self).__init__()
        self.out_channels = out_channels
        self.cutoff = cutoff
...
class ComENet(nn.Module):
    ...
    def forward(self, z, pos, batch):
        return self._forward(z, pos, batch)
    
    
    
if __name__ == '__main__':
    
    net = ComENet(out_channels=3)
    
    pos = torch.tensor([[ 1.2186364,  -0.17240895, -0.35873044],
                      [ 0.02923702, -0.17427893,  0.5967226 ],
                      [-1.2410389,   0.3059502,  -0.09850681]], dtype=torch.float32)
    z = torch.tensor([6,6,6], dtype=torch.long)
    batch = torch.tensor([0,0,0], dtype=torch.long)
    
    x, emb = net.encode(z, pos, batch)
