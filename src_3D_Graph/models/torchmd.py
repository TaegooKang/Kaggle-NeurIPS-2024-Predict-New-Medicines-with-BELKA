"""
TorchMD-Net implementation.
Source from 'https://github.com/torchmd/torchmd-net/blob/main/torchmdnet/models/torchmd_et.py'
"""
...
def create_model(args, prior_model=None, mean=None, std=None):
    ...
    # )
    output_model = EquivariantVectorOutput(
        args["embedding_dimension"],
        activation=args["activation"],
        reduce_op=args["reduce_op"],
    )
...
class GatedEquivariantBlock(nn.Module):
    ...
    def __init__(
        ...
        self.update_net = nn.Sequential(
            ...
            act_class(),
            nn.Linear(intermediate_channels, 2 * self.out_channels),
        )
    ...
    def forward(self, x, v):
        ...
        x, v = torch.split(x, self.out_channels, dim=-1)
...
class TorchMD_ET(nn.Module):
    ...
    Args:
        ...
        max_num_neighbors (int, optional): Maximum number of neighbors to return for a
            given node/atom when constructing t . .   he molecular graph during forward passes.
...
def build_torchmdnet():
    print('TorchMD-Net initializing...')
    # For QM9 configuration
    with open('torchmd_config.yaml', 'r') as r:
        config = yaml.safe_load(r)
    config["prior_args"] = {'max_z': 120}
